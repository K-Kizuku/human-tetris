...
class GameCore: ObservableObject {
    ...
    private func startDropTimer() {
        ...
        print("GameCore: Starting drop timer with interval \(dropInterval)")
        dropTimer = Timer.scheduledTimer(withTimeInterval: dropInterval, repeats: true) { [weak self] _ in
    ...
    func spawnPiece(_ piece: Polyomino, at column: Int) {
        print("GameCore: spawnPiece called with piece size=\(piece.size), width=\(piece.width), column=\(column)")
    ...
    func hardDrop() {
        ...
        var dropDistance = 0
        while gameState.isValidPosition(piece: piece, at: (
    ...
    func calculateScore(iou: Float, stableTime: Float, linesCleared: Int, diversityIndex: Float) -> Int {
        let alpha: Float = 4.0
        let beta: Float = 10.0
        let gamma: Float = 3.0
        let delta: Float = 5.0
